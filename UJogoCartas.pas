...
begin
   ...
   begin
      //varre todos os componentes do formulário
      for iIndice := 0 to ComponentCount - 1 do
      begin
         sName := Components[iIndice].Name;
         //se o componente for uma imagem com o início do nome abaxio (o restante é a numeração da imagem)
         //então é um objeto de imagem para ARcano Maior
         if (Copy(sName, 1, 11) = 'imgArcMaior') then
         begin
            iNumCard := StrToInt(Copy(sName,length(sName)-1,2));
            //Se for a imagem com o número escolhido, muda a imagem de fundo e muda a posição da carta na tela,
