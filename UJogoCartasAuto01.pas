...
type
  TFJogoCartasAuto01 = class(TForm)
  ...
  private
    ...
    gsDir: string; //diretório da aplicação
...
var
  FJogoCartasAuto01: TFJogoCartasAuto01;
...
//----------------------------------------------------
procedure TFJogoCartasAuto01.FormCreate(Sender: TObject);
begin
   giContMaior := 0;
   giContMenor := 0;
   gbJogou     := False;
...
//----------------------------------------------------
procedure TFJogoCartasAuto01.FormShow(Sender: TObject);
var liCont: integer;
begin
...
//----------------------------------------------------
procedure TFJogoCartasAuto01.btnLimparClick(Sender: TObject);
...
//----------------------------------------------------
procedure TFJogoCartasAuto01.CriarImagesMaior(imgBase: TImage; sName: string; iNum, iCont: integer);
var iNewImage: TImage;
begin
   iNewImage := TImage.Create(FJogoCartasAuto01);
...
// Cria dinamicamente os objetos de magem das cartas
//----------------------------------------------------
procedure TFJogoCartasAuto01.CriarImagesMenor(imgBase: TImage; sName: string; iNum, iCont: integer);
var iNewImage: TImage;
begin
   iNewImage := TImage.Create(FJogoCartasAuto01);
   iNewImage.Visible    := True;
...
//--------------------------------------------------------
procedure TFJogoCartasAuto01.ImageCartaMaiorClick(Sender: TObject);
begin
...
// Quando clicar em uma imagem, destacar a imagem na tela
//--------------------------------------------------------
procedure TFJogoCartasAuto01.ImageCartaMenorClick(Sender: TObject);
begin
   if ((Sender as TImage).Hint = '') then
   begin
      MessageDlg('Embaralhe as cartas primeiro!',mtWarning,[mbOk],0);
...
//--------------------------------------------------------
procedure TFJogoCartasAuto01.btnAnalisarClick(Sender: TObject);
begin
   gbStop := True;
   Exit;
...
procedure TFJogoCartasAuto01.EmbaralharAutomaticamente(Sender: TObject);
var iCont, iNumCard: integer;
    sCard, sName:string;
var liInd, i,j:integer;
    s: string;
    bExiste: Boolean;
    ArrayMaior:array [0..5] of integer;
    ArrayMenor:array [0..5] of integer;
begin
   for liInd := 1 to SpinEdit1.Value do
   begin
   ...
   begin
      ...
      begin
         ...
         try
            sName := 'imgArcMaior'+s;
            ImageCartaMaiorClick((FindComponent(sName) as TImage));
         except
            on E : Exception do
      ...
      begin
         ...
         try
            sName := 'imgArcMenor'+s;
            ImageCartaMenorClick((FindComponent(sName) as TImage));
         except
            on E : Exception do
            ShowMessage(E.ClassName+' error raised, with message : '+E.Message);
         end;
...
procedure TFJogoCartasAuto01.btnGravarClick(Sender: TObject);
var lsSQL: string;
    liIdJogo: longint;
begin
   if not(gbJogou) then
   begin
      MessageDlg('Efetue um novo jogo de tarô!',mtWarning,[mbOk],0);
      Exit;
   end;
   ...
   try
      ...
      //primeiro grava a partida e seta a variável para não gravar nos próximos jogos. Guarda o Id
